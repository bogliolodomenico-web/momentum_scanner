...
hide_streamlit_style = """
    <style>
        ...
        /* Pulsante laterale nella sidebar */
...
def download_and_analyze(ticker: str, min_conditions: int) -> dict:
    try:
        ...
        gate_score = (int(bool(last['C5_EMA'])) + int(bool(last['C8_TREND'])) +
                      int(bool(last['LIQUIDITY_OK'])) + int(psar_ok))
        return {
            "error": None,
...
def _bar_html(score: int, max_score: int, label: str) -> str:
    ...
    return (f'<div style="margin-top:0.45rem;">'
            ...
            f'<div style="height:6px;border-radius:4px;width:{pct}%;{fill}"></div></div></div>')

def render_ticker_card(ticker: str, nome: str, data: dict, changed: bool = False):
    if data.get("error"):
        st.markdown(f'<div class="card-sell"><span class="ticker-name">{ticker}</span>'
                    f'<span class="badge-sell">ERRORE</span>'
                    f'<div style="color:#6e4040;font-size:0.8rem;margin-top:0.5rem;">{data["error"]}</div></div>',
    ...
    if data.get("rt_today"):
        t_html = f'<span style="color:#1a1a1a;font-size:0.7rem;font-family:monospace;">🔴 LIVE {data["rt_time"]}</span>'
    elif data.get("rt_time"):
        t_html = f'<span style="color:#4a4a4a;font-size:0.7rem;font-family:monospace;">📅 {data["rt_time"]}</span>'
    else:
        t_html = f'<span style="color:#4a4a4a;font-size:0.7rem;font-family:monospace;">📅 chiusura {data["date"]}</span>'
    ytd_html = ""
...
with st.sidebar:
    ...
    st.caption("Definisce quante **condizioni su 6** devono essere soddisfatte per generare un segnale BUY.")
    ...
    risk_desc = {
        ...
        "⚡ Rischio Massimo": "Bastano 4 condizioni su 6. Più segnali ma più falsi positivi.",
    }
    st.info(risk_desc[risk_level])
    st.divider()
    st.markdown("### 🎯 Parametri di Uscita")
    tp_pct = st.slider("Take Profit", 10, 100, 50, 5, "%d%%")
    ts_start = st.slider("Trailing Stop — attivazione", 1, 20, 3, 1, "%d%%")
    ts_post = st.slider("Trailing Stop — ampiezza", 5, 30, 12, 1, "%d%%")
    max_days = st.number_input("Max giorni in posizione", 30, 1000, 600, 10)
    st.divider()
    ...
    st.markdown(
        ...
        f'RSI [{CFG_BASE["RSI_MIN"]},{CFG_BASE["RSI_MAX"]}] &uarr;{CFG_BASE["RSI_SLOPE_BARS"]}gg<br>'
        ...
        f'Periodo scaricato: {REQUIRED_DAYS} gg</div>',
        unsafe_allow_html=True
    )
    st.divider()
    ...
    if auto_refresh:
        ...
        if mkt_open:
            ...
            st.warning("Mercato chiuso — dati statici")
...
st.markdown(
    ...
    "Non costituisce consulenza finanziaria</div>",
    unsafe_allow_html=True
)
