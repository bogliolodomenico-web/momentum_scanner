...
hide_streamlit_style = """
    <style>
        ...
        /* Manteniamo l'header presente nel DOM ma completamente trasparente */
        ...
        .badge-sell { background-color: #da3633; color: white; }
        .badge-warn { background-color: #d29922; color: #1a1a1a; }
        .badge-changed { background-color: #1f6feb; color: white; }
        ...
        .scanner-header h1 { color: #3a2a1f; }
        .scanner-header p { color: #5a4a3a; }
        
        .card-buy .ticker-name, .card-sell .ticker-name { color: #1a1a1a; }
...
def build_indicators(df_d: pd.DataFrame, df_w: pd.DataFrame) -> pd.DataFrame:
    ...
    macd_obj    = ta.trend.MACD(d['Close'], window_fast=CFG_BASE["MACD_FAST"], window_slow=CFG_BASE["MACD_SLOW"], window_sign=CFG_BASE["MACD_SIGNAL"])
    ...
    stoch        = ta.momentum.StochasticOscillator(d['High'], d['Low'], d['Close'], window=CFG_BASE["STOCH_K_PERIOD"], smooth_window=CFG_BASE["STOCH_SMOOTH"])
    ...
    w_macd       = ta.trend.MACD(w['Close'], window_fast=CFG_BASE["W_MACD_FAST"], window_slow=CFG_BASE["W_MACD_SLOW"], window_sign=CFG_BASE["W_MACD_SIGNAL"])
...
def apply_conditions(df: pd.DataFrame, min_conditions: int) -> pd.DataFrame:
    ...
    d['C3_STOCH']  = ((d['STOCH_K'] > d['STOCH_D']) & (d['STOCH_K'] < CFG_BASE["STOCH_MAX"]) & (d['STOCH_K'] > CFG_BASE["STOCH_MIN"])).fillna(False)
    d['C4_RSI']    = ((d['RSI'] >= CFG_BASE["RSI_MIN"]) & (d['RSI'] <= CFG_BASE["RSI_MAX"]) & (d['RSI_SLOPE'] > 0)).fillna(False)
    d['C5_EMA']    = ((d['Close'] > d['EMA']) & (d['EMA_DIST'] <= CFG_BASE["EMA_MAX_DIST"])).fillna(False)
    ...
    d['SCORE'] = (d['C1_MACD'].astype(int) + d['C2_W_MACD'].astype(int) + d['C3_STOCH'].astype(int) + d['C4_RSI'].astype(int) + d['C6_VOL'].astype(int) + d['C7_MOM'].astype(int))
    d['SETUP_ACTIVE'] = (d['LIQUIDITY_OK'] & d['C5_EMA'] & d['C8_TREND'] & (d['SCORE'] >= min_conditions) & (d['PSAR_TREND'].fillna(1) != -1))
    return d
...
def download_and_analyze(ticker: str, min_conditions: int) -> dict:
    try:
        ...
        df_d = yf.download(ticker, start_date, end=end_date, progress=False, auto_adjust=False, interval='1d')
        df_w = yf.download(ticker, start_date, end=end_date, progress=False, auto_adjust=False, interval='1wk')
        ...
        if df.empty: return {"error": "Indicatori vuoti"}
        ...
        try:
            df_live = yf.download(ticker, period="2d", interval="1m", progress=False, auto_adjust=False)
            ...
            if not df_live.empty:
                ...
                if bar_ts.tzinfo is None: bar_ts = bar_ts.tz_localize('UTC')
                rt_time  = bar_ts.tz_convert('Europe/Rome').strftime('%d/%m/%Y %H:%M')
        except Exception:
            pass

        psar_ok    = bool(last['PSAR_TREND'] == 1)
        gate_score = int(bool(last['C5_EMA'])) + int(bool(last['C8_TREND'])) + int(bool(last['LIQUIDITY_OK'])) + int(psar_ok)
        ...
        return {
            ...
            "rt_price":     rt_price,
            "rt_time":      rt_time,
            "rt_today":     (rt_time is not None and rt_time.startswith(datetime.now().strftime('%d/%m/%Y'))),
...
def _bar_html(score: int, max_score: int, label: str) -> str:
    ...
    if score == max_score: fill = "background:linear-gradient(90deg,#238636,#3fb950)"
    elif score >= max_score * 0.6: fill = "background:linear-gradient(90deg,#d29922,#e3b341)"
    else: fill = "background:linear-gradient(90deg,#da3633,#f85149)"
    return (f'<div style="margin-top:0.45rem;">'
            f'<div style="font-size:0.72rem;color:#4a4a4a;font-family:IBM Plex Mono,monospace;margin-bottom:3px;">{label} {score}/{max_score}</div>'
            f'<div style="background:#e0e0e0;border-radius:4px;height:6px;width:100%;overflow:hidden;">'
...
def render_ticker_card(ticker: str, nome: str, data: dict, changed: bool = False):
    if data.get("error"):
        st.markdown(f'<div class="card-sell"><span class="ticker-name">{ticker}</span><span class="badge-sell">ERRORE</span><div style="color:#6e4040;font-size:0.8rem;margin-top:0.5rem;">{data["error"]}</div></div>', unsafe_allow_html=True)
        return
    ...
    if changed: base_cls += " card-changed"
    changed_badge = '<span class="badge-changed">⚡ CAMBIO</span>' if changed else ""
    px = data["rt_price"] if data.get("rt_price") else data["price"]

    if data.get("rt_today"): t_html = f'<span style="color:#1a1a1a;font-size:0.7rem;font-family:monospace;">🔴 LIVE {data["rt_time"]}</span>'
    elif data.get("rt_time"): t_html = f'<span style="color:#4a4a4a;font-size:0.7rem;font-family:monospace;">📅 {data["rt_time"]}</span>'
    else: t_html = f'<span style="color:#4a4a4a;font-size:0.7rem;font-family:monospace;">📅 chiusura {data["date"]}</span>'
    ...
    st.markdown(
        ...
        f'<div style="display:flex;align-items:center;flex-wrap:wrap;"><span class="ticker-name">{ticker}</span><span style="color:#4a4a4a;font-size:0.85rem;margin-left:8px;">{nome}</span>{badge}{changed_badge}</div>'
        f'<div style="margin-top:0.4rem;display:flex;align-items:baseline;flex-wrap:wrap;gap:4px;"><span style="font-family:IBM Plex Mono,monospace;font-size:1.25rem;color:#1a1a1a;font-weight:700;">{px:.3f} €</span>{ytd_html}{w52_html}</div>'
...
with st.sidebar:
    ...
    risk_level = st.radio("Profilo", options=["🛡️ Rischio Minimo", "⚖️ Rischio Normale", "⚡ Rischio Massimo"], index=1)
    risk_map       = {"🛡️ Rischio Minimo": 6, "⚖️ Rischio Normale": 5, "⚡ Rischio Massimo": 4}
    min_conditions = risk_map[risk_level]
    risk_desc      = {
        "🛡️ Rischio Minimo":  "Tutte e 6 le condizioni devono essere OK. Segnali molto selettivi.",
        "⚖️ Rischio Normale": "5 condizioni su 6. Bilanciamento ottimale.",
        "⚡ Rischio Massimo": "Bastano 4 condizioni su 6. Più opportunità ma più falsi positivi.",
    }
    st.info(risk_desc[risk_level])

    st.divider()
    st.markdown("### 🎯 Parametri di Uscita")
    tp_pct = st.slider("Take Profit", min_value=10, max_value=100, value=50, step=5, format="%d%%")
    ts_start = st.slider("Trailing Stop — attivazione", min_value=1, max_value=20, value=3, step=1, format="%d%%")
    ts_post = st.slider("Trailing Stop — ampiezza", min_value=5, max_value=30, value=12, step=1, format="%d%%")
    max_days = st.number_input("Max giorni in posizione", min_value=30, max_value=1000, value=600, step=10)

    st.divider()
    st.markdown("### 📋 Riepilogo Attivo")
    st.markdown(
        f'<div class="params-box"><strong>— ENTRY —</strong><br>EMA({CFG_BASE["EMA_PERIOD"]}) dist &le;{CFG_BASE["EMA_MAX_DIST"]:.0f}%<br>TrendScore &gt; {CFG_BASE["LIM_TRENDSCORE"]}<br>Stoch [{CFG_BASE["STOCH_MIN"]},{CFG_BASE["STOCH_MAX"]}] K&gt;D<br>RSI [{CFG_BASE["RSI_MIN"]},{CFG_BASE["RSI_MAX"]}] &uarr;{CFG_BASE["RSI_SLOPE_BARS"]}gg<br>GATE: EMA + Trend + Liquidit&agrave; + PSAR (4/4)<br>Min condizioni: <strong>{min_conditions}/6</strong><br>Liquidit&agrave;: &ge;{CFG_BASE["MIN_TURNOVER_EUR"]/1e6:.0f}M&euro;/gg<br><br><strong>— EXIT —</strong><br>Take Profit: +{tp_pct}%<br>Trail attivazione: -{ts_start}%<br>Trail ampiezza: -{ts_post}%<br>Max durata: {max_days} gg<br><br><strong>— DATI —</strong><br>Periodo scaricato: {REQUIRED_DAYS} gg</div>',
        unsafe_allow_html=True
    )

    st.divider()
    st.markdown("### 🔄 Aggiornamento")
    auto_refresh = st.checkbox("Auto-aggiornamento", value=False)
    refresh_mins = 30
    if auto_refresh:
        refresh_mins = st.select_slider("Intervallo (minuti)", options=[5, 10, 15, 20, 30, 60], value=30)
        if mkt_open: st.success(f"⏱ Prossimo aggiornamento tra {refresh_mins} min")
        else: st.warning("Mercato chiuso — dati statici")
...
if not avvia and not auto_refresh:
    st.markdown('<div style="text-align:center;padding:3rem;color:#4d5566;"><div style="font-size:2.5rem;margin-bottom:0.5rem;">📡</div><div style="font-family:IBM Plex Mono,monospace;font-size:0.9rem;">Configura i parametri nel pannello laterale<br>e premi <strong style="color:#58a6ff;">ANALIZZA SEGNALI</strong></div></div>', unsafe_allow_html=True)
    st.stop()
...
for ticker, data in results_map.items():
    if not data.get("error"):
        ...
        if ticker in prev and prev[ticker] != new_state: changed_tickers.add(ticker)
...
def sort_key(item):
    t, d = item
    if d.get("error"): return (9, 0, t)
    chg = 0 if t in changed_tickers else 1
    if ordine == "Setup ON prima → Score": return (chg, 0 if d["setup_active"] else 1, -d["score"], t)
    elif ordine == "Score decrescente": return (chg, -d["score"], t)
    else: return (0, 0, t)
...
st.markdown('<div class="section-title">SEGNALI PER TICKER</div>', unsafe_allow_html=True)
...
with col_l:
    for ticker, data in left_items:
        render_ticker_card(ticker, ALL_TICKERS.get(ticker, ticker), data, changed=(ticker in changed_tickers))
with col_r:
    for ticker, data in right_items:
        render_ticker_card(ticker, ALL_TICKERS.get(ticker, ticker), data, changed=(ticker in changed_tickers))

st.divider()
st.markdown("<div style='text-align:center;color:#4d5566;font-size:0.72rem;font-family:monospace;'>Momentum Setup Scanner v3.6.1 · Solo a scopo informativo · Non costituisce consulenza finanziaria</div>", unsafe_allow_html=True)
